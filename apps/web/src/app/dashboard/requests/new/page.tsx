  ...
  return (
    <RequireAuth>
      <div className="max-w-3xl">
        ...
        <h1 className="mt-4 text-2xl font-semibold">New request</h1>
        ...
        {tab === 'file' && (
          <div className="mt-4 space-y-4 rounded-lg border border-slate-800 bg-slate-900 p-4">
            <label className="block text-sm">
              <span className="mb-1 block text-slate-400">Paste text</span>
              <textarea
                rows={5}
                value={rawText}
            ...
            <label className="block text-sm">
              <span className="mb-1 block text-slate-400">Or upload file / photo</span>
              <input
                type="file"
        ...
        <form onSubmit={onSave} className="mt-6 space-y-4">
          {parser && (
            <p className="text-xs text-slate-500">Parsed with: {parser} — review lines before saving.</p>
          )}
          <input
            placeholder="Title"
            value={title}
          ...
          <div className="space-y-3">
            ...
            {lines.map((line, i) => (
              <div key={i} className="grid gap-2 rounded-md border border-slate-800 p-3 sm:grid-cols-2">
                <input
                  placeholder="Raw line / description"
                  value={line.rawLine ?? ''}
