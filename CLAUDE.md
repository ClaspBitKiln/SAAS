...
## Разделение ролей: Claude РУКОВОДИТ, Cursor ИСПОЛНЯЕТ (правило владельца, 2026-07-02)

**Claude НЕ пишет и НЕ правит код, конфиги, миграции; не деплоит; не делает git-операции.**
Claude только: review и RCA, архитектура и приоритеты, решения, **задания для Cursor**, память (Obsidian + `docs/`), проверка результата по CI.
Claude может править **только** `docs/**` и файлы памяти (NEXT, BUILD_STATUS, KNOWN_FAILURES, задания, ADR-заметки).

**Cursor исполняет всё остальное:** весь код, тесты, миграции, деплой, `git push`. По LOOP (CI = истина), One Failure At A Time.

**Цикл:** Claude готовит задание → Cursor исполняет → CI подтверждает (CI_GREEN) → Claude проверяет и обновляет память. Никакого кода от Claude, даже «мелкого фикса» — оформлять как задание Cursor.
...
## Finish What You Start (правило владельца, усилено 2026-07-02)

**Завершать ВСЕ начатые задачи до конца** (код → тесты → CI_GREEN или артефакт). Не бросать на полпути, не оставлять частично сделанным.
Остановка или отказ от задачи — **только по явной команде владельца** («отмени», «стоп», «не надо»). Пока владелец не отменил — доводить до DoD.
Блокер (например, зависший инструмент) — не повод бросить: переключить способ или оформить остаток как задание Cursor, зафиксировав в памяти.

## Постоянное разрешение владельца (2026-07-29)

Владелец заранее разрешил обычные действия по разработке SAAS: изменения в рамках
утверждённой задачи, ветки, коммиты, push, pull request, CI, минимальные исправления
ошибок, merge после зелёного CI и безопасный деплой. Повторно спрашивать разрешение
на эти действия нельзя.

Новое подтверждение требуется только для платежей, передачи или раскрытия секретов,
сообщений реальным людям, необратимого удаления production-данных либо действий вне
поставленной задачи.

## Устойчивый рабочий контур

- Отсутствие локального `gh` не является блокером: `git` используется для веток,
  коммитов и push, подключённый GitHub-контур — для PR, CI и merge.
- Локальные проверки запускаются через `bash scripts/verify-local.sh`; команда хранит
  Corepack/pnpm-кэши в `.tool-cache`, а не в недоступном `/root/.cache`.
- После двух одинаковых технических сбоев способ меняется; один и тот же нерабочий
  путь не повторяется.
- Между сообщениями работа не выполняется сама по себе. Нельзя обещать фоновую
  разработку без реально созданной автоматизации.
- Загрузка большой клиентской базы отделена от разработки: код импорта и CI не
  зависят от production-авторизации, а данные загружаются позже пакетами с dry-run,
  дедупликацией и отчётом.
